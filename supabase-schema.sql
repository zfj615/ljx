...
VALUES 
  ...
  ('商业数据分析', '学习如何使用数据分析解决商业问题，包括市场分析、客户分析等', 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=Business%20data%20analysis%20course%20thumbnail%20with%20business%20charts%20and%20analytics&image_size=square_hd'),
  ('机器学习基础', '学习机器学习的基本概念和算法，包括监督学习、无监督学习等', 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=Machine%20learning%20basics%20course%20thumbnail%20with%20AI%20and%20data%20patterns&image_size=square_hd'),
  ('时间序列分析', '学习如何分析和预测时间序列数据，包括趋势分析、季节性分析等', 'https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=Time%20series%20analysis%20course%20thumbnail%20with%20time%20graphs%20and%20forecasting&image_size=square_hd');
...
-- 为商业数据分析课程添加章节和课时
INSERT INTO sections (course_id, title, order_index)
SELECT id, '商业数据分析基础', 1 FROM courses WHERE title = '商业数据分析';

INSERT INTO sections (course_id, title, order_index)
SELECT id, '市场分析', 2 FROM courses WHERE title = '商业数据分析';

INSERT INTO sections (course_id, title, order_index)
SELECT id, '客户分析', 3 FROM courses WHERE title = '商业数据分析';

-- 为机器学习基础课程添加章节和课时
INSERT INTO sections (course_id, title, order_index)
SELECT id, '机器学习简介', 1 FROM courses WHERE title = '机器学习基础';

INSERT INTO sections (course_id, title, order_index)
SELECT id, '监督学习', 2 FROM courses WHERE title = '机器学习基础';

INSERT INTO sections (course_id, title, order_index)
SELECT id, '无监督学习', 3 FROM courses WHERE title = '机器学习基础';

-- 为时间序列分析课程添加章节和课时
INSERT INTO sections (course_id, title, order_index)
SELECT id, '时间序列基础', 1 FROM courses WHERE title = '时间序列分析';

INSERT INTO sections (course_id, title, order_index)
SELECT id, '时间序列建模', 2 FROM courses WHERE title = '时间序列分析';

INSERT INTO sections (course_id, title, order_index)
SELECT id, '时间序列预测', 3 FROM courses WHERE title = '时间序列分析';
...
-- 章节1: 商业数据分析基础
INSERT INTO lessons (section_id, title, content, order_index)
SELECT s.id, '商业数据分析概述', '商业数据分析的基本概念和应用场景。', 1
FROM sections s
JOIN courses c ON s.course_id = c.id
WHERE c.title = '商业数据分析' AND s.title = '商业数据分析基础';

INSERT INTO lessons (section_id, title, content, order_index)
SELECT s.id, '数据驱动决策', '如何使用数据驱动商业决策。', 2
FROM sections s
JOIN courses c ON s.course_id = c.id
WHERE c.title = '商业数据分析' AND s.title = '商业数据分析基础';
...
-- 章节2: 市场分析
INSERT INTO lessons (section_id, title, content, order_index)
SELECT s.id, '市场趋势分析', '如何分析市场趋势和预测市场变化。', 1
FROM sections s
JOIN courses c ON s.course_id = c.id
WHERE c.title = '商业数据分析' AND s.title = '市场分析';

INSERT INTO lessons (section_id, title, content, order_index)
SELECT s.id, '竞争对手分析', '如何分析竞争对手的数据和策略。', 2
FROM sections s
JOIN courses c ON s.course_id = c.id
WHERE c.title = '商业数据分析' AND s.title = '市场分析';
...
-- 章节3: 客户分析
INSERT INTO lessons (section_id, title, content, order_index)
SELECT s.id, '客户行为分析', '如何分析客户行为和购买模式。', 1
FROM sections s
JOIN courses c ON s.course_id = c.id
WHERE c.title = '商业数据分析' AND s.title = '客户分析';

INSERT INTO lessons (section_id, title, content, order_index)
SELECT s.id, '客户细分', '如何根据数据对客户进行细分。', 2
FROM sections s
JOIN courses c ON s.course_id = c.id
WHERE c.title = '商业数据分析' AND s.title = '客户分析';
...
-- 章节1: 机器学习简介
INSERT INTO lessons (section_id, title, content, order_index)
SELECT s.id, '机器学习概念', '机器学习的基本概念和应用。', 1
FROM sections s
JOIN courses c ON s.course_id = c.id
WHERE c.title = '机器学习基础' AND s.title = '机器学习简介';

INSERT INTO lessons (section_id, title, content, order_index)
SELECT s.id, '机器学习工作流程', '机器学习项目的典型工作流程。', 2
FROM sections s
JOIN courses c ON s.course_id = c.id
WHERE c.title = '机器学习基础' AND s.title = '机器学习简介';
...
-- 章节2: 监督学习
INSERT INTO lessons (section_id, title, content, order_index)
SELECT s.id, '回归分析', '线性回归和逻辑回归的基本原理。', 1
FROM sections s
JOIN courses c ON s.course_id = c.id
WHERE c.title = '机器学习基础' AND s.title = '监督学习';

INSERT INTO lessons (section_id, title, content, order_index)
SELECT s.id, '分类算法', '决策树、随机森林等分类算法。', 2
FROM sections s
JOIN courses c ON s.course_id = c.id
WHERE c.title = '机器学习基础' AND s.title = '监督学习';
...
-- 章节3: 无监督学习
INSERT INTO lessons (section_id, title, content, order_index)
SELECT s.id, '聚类分析', 'K-means等聚类算法的应用。', 1
FROM sections s
JOIN courses c ON s.course_id = c.id
WHERE c.title = '机器学习基础' AND s.title = '无监督学习';

INSERT INTO lessons (section_id, title, content, order_index)
SELECT s.id, '降维技术', 'PCA等降维技术的应用。', 2
FROM sections s
JOIN courses c ON s.course_id = c.id
WHERE c.title = '机器学习基础' AND s.title = '无监督学习';
...
-- 章节1: 时间序列基础
INSERT INTO lessons (section_id, title, content, order_index)
SELECT s.id, '时间序列概念', '时间序列的基本概念和特征。', 1
FROM sections s
JOIN courses c ON s.course_id = c.id
WHERE c.title = '时间序列分析' AND s.title = '时间序列基础';

INSERT INTO lessons (section_id, title, content, order_index)
SELECT s.id, '时间序列预处理', '时间序列数据的预处理方法。', 2
FROM sections s
JOIN courses c ON s.course_id = c.id
WHERE c.title = '时间序列分析' AND s.title = '时间序列基础';
...
-- 章节2: 时间序列建模
INSERT INTO lessons (section_id, title, content, order_index)
SELECT s.id, 'ARIMA模型', 'ARIMA模型的原理和应用。', 1
FROM sections s
JOIN courses c ON s.course_id = c.id
WHERE c.title = '时间序列分析' AND s.title = '时间序列建模';

INSERT INTO lessons (section_id, title, content, order_index)
SELECT s.id, '季节性模型', '季节性时间序列模型的应用。', 2
FROM sections s
JOIN courses c ON s.course_id = c.id
WHERE c.title = '时间序列分析' AND s.title = '时间序列建模';
...
-- 章节3: 时间序列预测
INSERT INTO lessons (section_id, title, content, order_index)
SELECT s.id, '时间序列预测方法', '常用的时间序列预测方法。', 1
FROM sections s
JOIN courses c ON s.course_id = c.id
WHERE c.title = '时间序列分析' AND s.title = '时间序列预测';

INSERT INTO lessons (section_id, title, content, order_index)
SELECT s.id, '预测评估', '如何评估时间序列预测模型的性能。', 2
FROM sections s
JOIN courses c ON s.course_id = c.id
WHERE c.title = '时间序列分析' AND s.title = '时间序列预测';
